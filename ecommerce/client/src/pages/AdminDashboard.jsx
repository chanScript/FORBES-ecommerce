...
export default function AdminDashboard() {
  ...
  const { data: trashListings, isLoading: trashLoading } = useQuery({
    queryKey: ['admin', 'trash'],
    queryFn: () => adminAPI.listTrash().then(r => r.data),
  });

  const { data: submissionCountData, isLoading: subLoading } = useQuery({
    queryKey: ['admin', 'submissionCount'],
    queryFn: () => adminAPI.pendingSubmissionCount().then(r => r.data),
  });
  ...
  const totalCount = allListings?.pagination?.total || 0;
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 lg:px-8">
      ...
      {submissionCount > 0 && (
        <div className="mt-4 rounded-xl border border-primary-accent/30 bg-primary-accent/5 p-6">
          <h2 className="flex items-center gap-2 text-lg font-semibold text-gray-900">
            <ClipboardList className="h-5 w-5 text-primary-accent" />
            {submissionCount} Pending Seller Submissions
          </h2>
          <p className="mt-1 text-sm text-secondary-muted">
            Anonymous sellers have submitted properties for review. Convert them to listings.
          </p>
          <Link to="/admin/submissions" className="btn-primary mt-4 inline-block">
            Review Submissions
          </Link>
        </div>
      )}
      ...
      {inquiryCount > 0 && (
        <div className="mt-4 rounded-xl border border-primary-blue/30 bg-primary-blue/5 p-6">
          ...
          <p className="mt-1 text-sm text-secondary-muted">
            Customers are interested in your listings. Follow up to close leads.
          </p>
